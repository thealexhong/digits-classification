...
%%% YOUR CODE HERE %%%
GT = full(sparse(labels, 1:numImages, 1));
cost = -numImages_inv*(GT(:)'*log(probs(:))) + (lambda/2.)*(sum(Wd(:).^2)+sum(Wc(:).^2));
...
%%% YOUR CODE HERE %%%
delta = -(GT - probs);
delta_pooling = reshape(Wd'*delta, outputDim, outputDim, numFilters, numImages);
delta_convolution = zeros(convDim,convDim,numFilters,numImages);
% upsampling the delta_pool to delta_conv
for i=1:numImages
    for j=1:numFilters
        delta_convolution(:,:,j,i) = (1./poolDim^2) .* kron(squeeze(delta_pooling(:,:,j,i)), ones(poolDim));
    end
end
delta_convolution = activations .* (1-activations) .* delta_convolution;
